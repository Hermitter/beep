...
import (
	...
	"github.com/Hermitter/beep/mp3"
	"github.com/Hermitter/beep/speaker"
)
...
Before we get into the hard-core composing and controlling, I'll teach you how you can observe a streamer, if it is a [`beep.StreamSeeker`](https://godoc.org/github.com/Hermitter/beep#StreamSeeker). Namely, how you can check it's current playing position.
...
Making new streamers from old ones, that's a common pattern in Beep. We've already seen it with [`beep.Resample`](https://godoc.org/github.com/Hermitter/beep#Resample) and [`beep.Seq`](https://godoc.org/github.com/Hermitter/beep#Seq). The new streamer functions by using the old streamer under the hood, somehow manipulating its samples.

Another such streamer is [`beep.Loop`](https://godoc.org/github.com/Hermitter/beep#Loop). It's a very simple one. Keep all the code as is, just add the `loop := ...` line and edit `speaker.Play` to play the `loop`:
...
Maybe your song is too long and it takes too much to finish. No problemo! We can speed it up with [`beep.ResampleRatio`](https://godoc.org/github.com/Hermitter/beep#ResampleRatio) (the first argument, `4`, is once again the quality index):
...
We'll take a look at one more streamer, this time not from the `beep` package, but from the extra [`"github.com/Hermitter/beep/effects"`](https://godoc.org/github.com/Hermitter/beep/effects) package. The _effect_ we'll be dealing with is simple: changing volume.
...
The [`effects.Volume`](https://godoc.org/github.com/Hermitter/beep/effects#Volume) streamer is a struct, just like `beep.Ctrl`, and looks like this:
...
But well, just increasing the volume might not be enough a punishment. Let's also speed up the playback, that'll show them! So, we'll additionally wrap with [`beep.ResampleRatio`](https://godoc.org/github.com/Hermitter/beep#ResampleRatio):
...
Look into the documentation of both [`beep`](https://godoc.org/github.com/Hermitter/beep) and [`effects`](https://godoc.org/github.com/Hermitter/beep/effects) package to find other useful streamers.